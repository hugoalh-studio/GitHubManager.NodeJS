...
function labelReplaceLocal(sourceData) {
	const localStorage = require("../../internal/local-storage.js");
	localStorage.write("label", sourceData);
};
async function labelReplaceNetwork(sourceData, targetRepositories) {
	const labelAdd = require("./add-one.js"),
		labelDelete = require("./delete-one.js"),
	...
	for (let index = 0; index < targetRepositories.length; index++) {
		...
		if (advancedDetermine.isArray(dataDelete) === true) {
			await labelDelete(targetRepository, ...dataDelete);
		};
...
async function labelReplace(sourceRepository, ...targetRepositories) {
	...
	if (targetLocalStorage === true) {
		labelReplaceLocal(sourceData);
	};
	if (targetNetworkRepositoriesQueue.length > 0) {
		return labelReplaceNetwork(sourceData, targetNetworkRepositoriesQueue);
	};
};
module.exports = labelReplace;
