...
async function secretListNetwork(repositoryOwner, repositoryName, internalUse) {
	...
	for (let index = 0; index < totalPage; index++) {
		let data = await githubREST.actions.listRepoSecrets({
		...
		if (index === 0) {
			if (advancedDetermine.isString(data.headers.link) === true) {
				if (data.headers.link.search(/\?page=(\d+)&per_page=100>; rel="last"/giu) !== -1) {
					...
					console.log(`${chalk.bgBlue.white.bold("INFO")} Repository "${repositoryOwner}/${repositoryName}" has at most ${totalPage * 100} secrets, this action may take longer than usual!`);
				};
			};
		};
	};
...
async function secretListOrganization(organizationName, internalUse) {
	...
	let result = [],
		totalPage = 1;
	for (let index = 0; index < totalPage; index++) {
		let data = await githubREST.actions.listOrgSecrets({
		...
		if (index === 0) {
			if (advancedDetermine.isString(data.headers.link) === true) {
				if (data.headers.link.search(/\?page=(\d+)&per_page=100>; rel="last"/giu) !== -1) {
					let totalPageData = data.headers.link.match(/\?page=(\d+)&per_page=100>; rel="last"/giu)[0].split("&")[0].split("=")[1];
					totalPage = Number(totalPageData);
					console.log(`${chalk.bgBlue.white.bold("INFO")} Organization "${organizationName}" has at most ${totalPage * 100} secrets, this action may take longer than usual!`);
				};
			};
		};
	};
