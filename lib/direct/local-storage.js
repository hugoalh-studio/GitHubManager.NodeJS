...
function directLocalStorage(commandLine) {
	...
	if (method.search(/^decrypt(ion)?$/giu) === 0) {
		return require("../action/local-storage/decryption.js")(commandLine);
	};
	if (method.search(/^encrypt(ion)?$/giu) === 0) {
	...
	internalConsole.error(`${languageService.errorUnknownCommand_1}local-storage ${method}${languageService.errorUnknownCommand_2}`);
