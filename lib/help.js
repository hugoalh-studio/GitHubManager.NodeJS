...
module.exports = function main() {
	...
	[
		...
		[`label pullmerge {target::repository/localstorage} ...{source::repository/localstorage}`, `Pull all of the labels from source repository(ies) or local storage to target repository or local storage.`],
		[`label pushmerge {source::repository/localstorage} ...{target::repository/localstorage}`, `Push all of the labels from source repository or local storage to target repository(ies) or local storage.`],
		...
		[`${chalk.bgMagenta.white.bold("W.I.P.")} secret merge {source::repository/organization} ...{target::repository/organization}`, `Merge all of the secrets from source repository or organization to target repository(ies) or organization(s).`],
		[`${chalk.bgMagenta.white.bold("W.I.P.")} secret replace {source::repository/organization} ...{target::repository/organization}`, `Replace all of the secrets from source repository or organization to target repository(ies) or organization(s).`]
	].forEach((element) => {
		commandListBuild.div(
			{
				text: `-`,
