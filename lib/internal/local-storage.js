...
function localStorageRead(file) {
	try {
		...
		internalConsole.error("Operating system denied application read from application storage file \"%file%\"!", { file: file });
		process.exit(0);
	};
};
function localStorageReadBase64(file) {
	try {
		...
		internalConsole.error("Operating system cannot decode application storage file \"%file%\"!", { file: file });
		process.exit(0);
	};
};
function localStorageWrite(file, data) {
	try {
		...
		internalConsole.error("Operating system denied application write to application storage file \"%file%\"!", { file: file });
		process.exit(0);
	};
};
function localStorageWriteBase64(file, data) {
	try {
		...
		internalConsole.error("Operating system cannot encode application storage file \"%file%\"!", { file: file });
		process.exit(0);
	};
};
...
try {
	...
	try {
		...
		internalConsole.error("Operating system denied application to access application storage directory!");
		process.exit(0);
	};
};
module.exports = {
