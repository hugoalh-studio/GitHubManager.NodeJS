...
async function network(owner, repositoryName, name, color, description) {
	...
	if (data.data.message) {
		console.error(`${chalk.bgRed.white.bold("ERROR")} ${data.data.message} (Code: L-A-${owner.toUpperCase()}-${repositoryName.toUpperCase()}-${name.toUpperCase()}-${color.toUpperCase()})! ([NodeJS] GitHub Manager)`);
		process.exit(0);
	};
};
