...
function read(type = "") {
	...
	switch (type.toLowerCase()) {
		...
		case "label":
			...
			break;
		default:
			console.error(`${chalk.bgRed.white.bold("ERROR")} Application error (Code: LS-R-${type.toUpperCase()})! Please report to author. ([NodeJS] GitHub Manager)`);
			process.exit(0);
			break;
	};
	let data;
...
function write(type = "", data = "") {
	...
	switch (type.toLowerCase()) {
		...
		case "label":
			...
			break;
		default:
			console.error(`${chalk.bgRed.white.bold("ERROR")} Application error (Code: LS-W-${type.toUpperCase()})! Please report to author. ([NodeJS] GitHub Manager)`);
			process.exit(0);
			break;
	};
	try {
