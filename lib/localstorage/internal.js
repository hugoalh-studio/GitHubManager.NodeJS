/*==================
[NodeJS] GitHub Manager - Local Storage - Internal
	Language:
		NodeJS/10.13.0
==================*/
...
function read(type, ignoreCheck = false) {
	let fileName;
	...
	if (type === "label") {
		if (dataHeader.length !== 3 || dataHeader[0] !== "name" || dataHeader[1] !== "color" || dataHeader[2] !== "description") {
			console.error(`${chalk.bgRed.white.bold("ERROR")} Storage file ${fileName} have invalid pattern, or data corrupted! ([NodeJS] GitHub Manager)`);
			process.exit(0);
		};
...
function write(type, data, ignoreCheck = false) {
	let fileName;
	switch (type.toLowerCase()) {
		case "account":
			fileName = fileNameAccount;
