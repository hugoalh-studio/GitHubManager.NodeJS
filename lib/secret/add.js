...
async function network(repositoryOwner, repositoryName, key, value) {
	const soulEncrypt = require("./encrypt.js"),
		...
		secretPublicKeyRepository = require("./publickeyrepository.js"),
		Octokit = require("@octokit/rest").Octokit;
	const octokit = new Octokit(passport);
	let publicKey = await secretPublicKeyRepository(repositoryOwner, repositoryName);
	...
	if (typeof data.data.message !== "undefined") {
		console.error(`${chalk.bgRed.white.bold("ERROR")} ${data.data.message}! ([NodeJS] GitHub Manager (S-A-${repositoryOwner.toUpperCase()}-${repositoryName.toUpperCase()}-${key.toUpperCase()}))`);
		process.exit(0);
	};
};
async function organization(organizationName, key, value) {
	const soulEncrypt = require("./encrypt.js"),
		passport = require("../passport.js")(),
		secretPublicKeyOrganization = require("./publickeyorganization.js"),
		Octokit = require("@octokit/rest").Octokit;
	const octokit = new Octokit(passport);
	let publicKey = await secretPublicKeyOrganization(organizationName);
	value = soulEncrypt(publicKey.key, value);
	...
	if (typeof data.data.message !== "undefined") {
		console.error(`${chalk.bgRed.white.bold("ERROR")} ${data.data.message}! ([NodeJS] GitHub Manager (S-A-${organizationName.toUpperCase()}-${key.toUpperCase()}))`);
		process.exit(0);
	};
};
